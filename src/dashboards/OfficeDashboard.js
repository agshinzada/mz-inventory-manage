...
function OfficeDashboard() {
  ...
  async function handleServiceItems() {
    try {
      const data = await getItemsByDepartment(14, 1);
  ...
  async function handleBuferItems() {
    try {
      const data = await getItemsByDepartment(19, 1);
      setBuferItems(data);
    } catch (error) {
      notifyError("Sistem xətası!");
      console.log(error);
  ...
  async function handleItems() {
    try {
      dispatch(getItems());
      setOfficeItems(items.filter((item) => item.LOCATION_ID === 1));
    } catch (error) {
      notifyError("Sistem xətası!");
      console.log(error);
    }
  ...
  async function handleNewItems() {
    try {
      const data = await getNewItems(1);
      setNewItems(data);
    } catch (error) {
      notifyError("Sistem xətası!");
      console.log(error);
  ...
  async function handleMonthlyFiches() {
    try {
      const data = await getFichesByMonth(1);
  ...
  async function handleCurrentAmount() {
    try {
      const data = await getTotalAmountByMonth(new Date().getMonth() + 1, 1);
  ...
  async function handlePreviousAmount() {
    try {
      const data = await getTotalAmountByMonth(new Date().getMonth(), 1);
  ...
  return (
    <div className="flex desktop:gap-8 mobile:gap-6 flex-wrap">
      <TitleBox title="ofis" />
...
export default memo(OfficeDashboard);
